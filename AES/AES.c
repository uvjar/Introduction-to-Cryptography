...
unsigned char gmul(unsigned char a, unsigned char b) {
	...
	for (i = 0; i < 8; i++) {
		...
		carry = a & (unsigned char)0x80;
		a <<= 1;
		if (carry) a ^= 0x1b; // 0000 0001 0001 1011	
		b >>= 1;
	}
...
void InvSubBytes(unsigned char * state){
	...
	for (int i =0; i< 4;i++){
		for (int j =0 ;j<4;j++){
			row = (state[4*i+j]&0xf0)>>4;
			col = state[4*i+j] & 0x0f;
			state[4*i+j] = InvSBox[16*row+col];
		}
	}
...
void MixColumns(unsigned char *state){
	unsigned char C[16] = {0x02,0x03,0x01,0x01, 0x01,0x02,0x03,0x01, 0x01,0x01,0x02,0x03, 0x03,0x01,0x01,0x02};
	...
	for(int j = 0; j<4; j++){
		for (int k = 0; k<4;k++){
			temp[k] = 0;
			for(int i =0; i<4; i++){
				temp[k] ^= gmul(C[4*k+i],state[4*j+i]);
			} 
		}
		for(int k =0;k<4;k++){
			state[4*j+k] = temp[k];
...
void InvMixColumns(unsigned char *state){
	unsigned char C[16] = {0x0E,0x0B,0x0D,0x09, 0x09,0x0E,0x0B,0x0D,
...
void KeyGeneration(unsigned char *InitialKey,unsigned char *key){
	...
	for (int i = 4;i < 4*(10+1);i++){
		...
		if(i%4==0){
			TopShift(temp);
			for (int i =0; i<4;i++){
				temp[i] = SBox[((temp[i]&0xf0)>>4)*16 + (temp[i]&0x0f)];
			}
			temp[0] ^= Rcon(i);
		}

		
		key[4*i+0] = key[4*(i-4)+0]^temp[0];
		key[4*i+1] = key[4*(i-4)+1]^temp[1];
		key[4*i+2] = key[4*(i-4)+2]^temp[2];
		key[4*i+3] = key[4*(i-4)+3]^temp[3];
...
void encryption(unsigned char *state, unsigned char *InitialKey){
    unsigned char key[4*4*11]={0};
    KeyGeneration(InitialKey,key);
...
void decryption(unsigned char *state, unsigned char *InitialKey){
